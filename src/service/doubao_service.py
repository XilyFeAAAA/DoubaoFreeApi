...
async def upload_file(file_type: int, file_name: str, file_data: bytes):
    ...
    logger.debug(f"开始上传文件: {file_name}, 类型: {file_type}, 大小: {len(file_data)} 字节, 使用会话: {session.name}")
    # ------ HEADERS -------
    DEFAULT_HEADERS = {
        'content-type': 'application/json',
        'cookie': session.cookie,
        # 'x-ms-token': session.x_ms_token,
    ...
    params = "&".join([
        ...
        "version_code=20800",
        f"web_id={session.web_id}",
        f"msToken={session.ms_token}",
        f"a_bogus={session.a_bogus}"
    ])
